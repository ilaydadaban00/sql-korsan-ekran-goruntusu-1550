...
CREATE TABLE IF NOT EXISTS `futbolcu` (
  ...
  `d_tarih` date DEFAULT NULL,
  `takim_id` int(11) DEFAULT NULL,
  `mevki` int(11) DEFAULT NULL,
  `forma_no` tinyint(4) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_futbolcu_futbolcu` (`takim_id`),
  CONSTRAINT `FK_futbolcu_futbolcu` FOREIGN KEY (`takim_id`) REFERENCES `futbolcu` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
